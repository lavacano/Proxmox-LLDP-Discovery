...
# Version: 9.3.0 (Portable AWK Hotfix)
#
# Changelog v9.3.0:
# - CRITICAL FIX (Syntax): Replaced all GNU-specific awk `match()` functions
#   with portable equivalents to fix compatibility issues with older awk versions.
# - CRITICAL FIX (Completeness): This version is a full, complete script file,
#   not a partial patch, to resolve "command not found" errors.
# =============================================================================
...
# ... (this part is unchanged from the last full script)
# --- Configuration ---
# ... (this part is unchanged from the last full script)
# --- Proxmox args ---
# ... (this part is unchanged from the last full script)
...
# NOTE: All functions from the previous full script (v9.1.0) are included here.
# Only the functions using the 'match()' command have been modified.

log_message() {
  local -r level="$1"; local -r message="$2"
...
get_tc_output_file() {
  local -r iface="$1"; local tmpf; tmpf=$(mktemp "${STATE_DIR}/tc-out.XXXXXX")
...
get_tc_mirrors_for_iface() {
    ...
    if [ "$TC_SUPPORTS_JSON" = true ]; then
        tc -j filter show dev "$iface" ingress 2>/dev/null | awk '
            BEGIN { RS = "}," }
            /mirred/ && /mirror/ {
                pos = index($0, "\"dev\"");
                if (pos > 0) {
                    ...
                    if (substr(line, 1, 1) == "\"") {
                        line = substr(line, 2);
                        end_quote = index(line, "\"");
                        if (end_quote > 0) {
                            print substr(line, 1, end_quote - 1);
                        }
                    }
                }
            }
    ...
    awk '
        ...
        {
            block=$0;
            if (block ~ /mirred/ && block ~ /mirror dev/) {
                pos = index(block, "mirror dev ");
                if (pos > 0) {
                    line = substr(block, pos + 11);
                    split(line, parts, " ");
                    print parts[1];
                }
            }
...
wait_for_interface() {
  local -r iface=$1; local -r max_wait=${2:-20}; local elapsed=0 delay=0.25
...
wait_for_rule() {
  local -r src=$1 dst=$2 want_exists=${3:-true}; local -r timeout=${4:-6}; local -r start=$(date +%s)
  while true; do
    get_running_state
...
save_handle_to_state() {
  local -r key="$1"; local -r val="$2"; local -r lockfile="${STATE_DIR}/${VMID}.lock"
...
read_state_value() {
  local -r key="$1"; if [ ! -f "$STATE_FILE" ]; then return 1; fi
...
add_rules_for() {
  ...
  local -r guest_if=$1 phys_if=$2; local -r net_id="${guest_if##*i}"
...
remove_rules_for() {
  ...
  if wait_for_rule "$phys_if" "$guest_if" false && wait_for_rule "$guest_if" "$phys_if" false; then
    ...
    local -r lockfile="${STATE_DIR}/${VMID}.lock"; exec 200>"$lockfile"; flock -x 200
...
# --- Main ---
log_message "INFO" "--- Stateful Hook v9.3.0 Started: VMID=$VMID, Phase=$PHASE ---"
